...
/* Retrain button special */
.retrain-btn > button {
    background: linear-gradient(135deg, #7c3aed, #4f46e5) !important;
}
.retrain-btn > button:hover {
...
COLORS = {
    ...
    "electricity": "#f59e0b",
    "cleanliness": "#10b981",
    "default": ["#3b82f6", "#06b6d4", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#ec4899"],
}
...
with st.sidebar:
    ...
    # Quick retrain available from anywhere
...
if page == "📊 Operations Hub":
    ...
    # ── Three-column layout: Charts | Prediction | Add ─
    ...
    with col_charts:
        ...
        with tab1:
            ...
            fig.add_trace(go.Bar(
                ...
                showlegend=False, name="Count"
            ), row=1, col=1)

            fig.add_trace(go.Bar(
                x=cat_avg["category"], y=cat_avg["avg_hours"].round(1),
                marker=dict(color=["#3b82f6", "#f59e0b", "#10b981"],
                            opacity=0.75,
                            line=dict(color="rgba(0,0,0,0)")),
                showlegend=False, name="Avg Hours"
            ), row=1, col=2)
        ...
        # Tab 3 – Distribution
        with tab3:
            fig = px.histogram(df, x="resolution_time_hours",
                nbins=30,
                color_discrete_sequence=["#3b82f6"],
                labels={"resolution_time_hours": "Resolution Time (hours)", "count": "Complaints"})
            fig.update_layout(**PLOTLY_LAYOUT, height=280,
                bargap=0.04)
            fig.update_traces(marker_line_width=0)
            st.plotly_chart(fig, use_container_width=True)

        # Tab 4 – Heatmap (day × category)
        with tab4:
            heat = df.pivot_table(
            ...
            fig = px.imshow(
                ...
                labels=dict(x="Day", y="Category", color="Avg Hours"),
                aspect="auto"
            )
            fig.update_layout(**PLOTLY_LAYOUT, height=280)
            st.plotly_chart(fig, use_container_width=True)

    # ── RIGHT: Prediction + Add Complaint ─────────────
    with col_right:
    ...
    # ── Bottom strip: Trend over time + box plots ──────
...
# ══════════════════════════════════════════════
    ...
    with k3:
        r2_pct = meta['r2_score'] * 100
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-label">R² Score</div>
            <div class="kpi-value">{meta['r2_score']}</div>
            <div class="kpi-sub">{r2_pct:.0f}% variance explained</div>
        </div>""", unsafe_allow_html=True)
    ...
    with left_col:
        ...
        # Sort by MAE, filter valid results
        ranked = sorted(
        ...
        if ranked:
            ...
            for rank, (name, scores) in enumerate(ranked, 1):
                ...
                bar_pct = max(5, int((1 - (scores["mae"] - best_mae_val) / (best_mae_val * 3 + 0.01)) * 100))
        ...
        if ranked:
            ...
            with chart_tab1:
                fig = go.Figure(go.Bar(
                    x=mae_list, y=names_list, orientation='h',
                    marker=dict(color=colors_bar, line=dict(color="rgba(0,0,0,0)")),
                    text=[f"{v}" for v in mae_list],
                    textposition="outside",
                    textfont=dict(color="#94a3b8", size=10)
                ))
                fig.update_layout(**PLOTLY_LAYOUT, height=420,
                    xaxis_title="Mean Absolute Error (hours)",
                    yaxis=dict(gridcolor="#1f2d45", linecolor="#1f2d45", tickcolor="#1f2d45",
            ...
            with chart_tab2:
                ...
                fig = go.Figure(go.Bar(
                    x=r2_list, y=names_list, orientation='h',
                    marker=dict(color=r2_colors, line=dict(color="rgba(0,0,0,0)")),
                    text=[f"{v}" for v in r2_list],
                    textposition="outside",
                    textfont=dict(color="#94a3b8", size=10)
                ))
                fig.update_layout(**PLOTLY_LAYOUT, height=420,
    ...
    # ── Model metadata detail + retrain section ────────
    ...
    with retrain_col:
        ...
        if needs_retrain:
            st.warning(f"⚠️ **{new_since_train} new complaints** recorded since last training. Retraining is recommended.")
        else:
